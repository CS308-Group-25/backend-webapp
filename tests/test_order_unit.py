...
#     # Commented out: strict transition rules have been removed.
#     # Arrange
#     order_repo = MagicMock()
#     order = MagicMock(spec=Order)
...
def test_update_status_same_status_calls_update():
    """Same-status updates now go through to the repo (no idempotent short-circuit)."""
    from datetime import datetime, timezone
...
def test_update_status_cancelled_order_raises_400():
    """Cancelled orders cannot have their status changed by the product manager."""
    order_repo = MagicMock()
    order = MagicMock(spec=Order)
...
def test_place_order_stock_depleted_after_payment_raises_409(
    ...
    product_repo.get_by_id_for_update.return_value = _make_product(stock=1)
